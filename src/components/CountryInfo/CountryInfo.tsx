...
const CountryInfo: React.FC<Props> = ({ selectedCountry }) => {
  ...
  const fetchData = useCallback(async () => {
    const { data: countryInfo } = await axios.get<IOneCountryInfo>(
      GetByCountryCode + selectedCountry,
    );
    ...
    selectedCountry && (
      <div className="col-8">
        <div className="currentCountry_Header d-flex p-3 justify-content-evenly">
          <div className="">
            <h2>
              <strong>Name:</strong> {currentCounty.name}
            </h2>
            <p>
              <strong>Capital:</strong> {currentCounty.capital}
            </p>
            <p>
              <strong>Population:</strong> {currentCounty.population}
            </p>
          </div>
          <img style={{ width: '200px' }} src={currentCounty.flag} alt="" />
        </div>
        <hr />
        <p>
          <strong>
            {currentCounty.borders
              ? 'Border Countries'
